...
/** Verify that the requesting user has the admin portal role. */
...
export async function GET(req: NextRequest) {
  ...
  const [usersRes, classesRes, postsRes, connectionsRes] = await Promise.all([
    fetch(`${DIRECTUS_URL}/users?fields[]=id,status&filter[status][_in]=active,suspended&limit=-1&aggregate[count]=id`,
